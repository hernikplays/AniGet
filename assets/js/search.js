...
function searchLink(link) {
    fetch(`https://trace.moe/api/search?url=${link}`)
        ...
        .then(data => {
            ...
            //If you DON'T want to make the name a link to auto search data, add // to the start of the following line
            document.getElementById("link").href = `https://hernikplays.github.io/AniGet/?search=${data.docs[0].title_english}`
            document.getElementById("items").style.visibility = "visible";
        })
        .catch(err => {
            document.getElementById("error").innerHTML = "Error: " + err;
